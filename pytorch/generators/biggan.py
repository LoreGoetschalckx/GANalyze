...
from generators.layers.batchnorm import SynchronizedBatchNorm2d as BatchNorm2d
...
class Generator512(nn.Module):
    def __init__(self, code_dim=128, n_class=1000, chn=96, debug=False):
        ...
        self.ScaledCrossReplicaBN = BatchNorm2d(1 * chn)
        self.colorize = SpectralNorm(nn.Conv2d(1 * chn, 3, [3, 3], padding=1))
...
def biggan256():
    s = os.path.join(os.path.dirname(os.path.realpath(__file__)),"biggan-256.pth")
    G = Generator256()
...
def biggan512():
    s = os.path.join(os.path.dirname(os.path.realpath(__file__)),"biggan-512.pth")
    G = Generator512()
    G.load_state_dict(torch.load(s))
    return G
